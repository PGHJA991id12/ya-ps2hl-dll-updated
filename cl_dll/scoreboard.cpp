...
bool CHudScoreboard :: Draw( float fTime )
{
	...
	if ( !gHUD.m_Teamplay ) 
		gHUD.DrawHudString( xpos, new_ypos, NAME_RANGE_MAX + xpos_rel, "player", 255, 140, 0 );
	else
		gHUD.DrawHudString( xpos, new_ypos, NAME_RANGE_MAX + xpos_rel, "Teams", 255, 140, 0 );
	...
	gHUD.DrawHudString(xpos_rel + 250, ypos, ScreenWidth, "damage", 255, 140, 0);
	gHUD.DrawHudString(xpos_rel + 310, ypos, ScreenWidth, "accuracy", 255, 140, 0);
	//gHUD.DrawHudString( PING_RANGE_MAX + xpos_rel - 35, ypos, ScreenWidth, "latency", 255, 140, 0 );
	*/

	gHUD.DrawHudStringReverse(KILLS_RANGE_MAX + xpos_rel - 50, new_ypos, 0, "kills", 255, 140, 0);
	//gHUD.DrawHudString(DIVIDER_POS + xpos_rel - 50, ypos, ScreenWidth, "/", 255, 140, 0);
	gHUD.DrawHudString(DIVIDER_POS + xpos_rel - 55, new_ypos, ScreenWidth, "/deaths", 255, 140, 0);
	//gHUD.DrawHudString(DEATHS_RANGE_MIN + xpos_rel - 45, ypos, ScreenWidth, "deaths", 255, 140, 0);
	gHUD.DrawHudString(xpos_rel + 185, new_ypos, ScreenWidth, "damage", 255, 140, 0);
	gHUD.DrawHudString(xpos_rel + 250, new_ypos, ScreenWidth, "accuracy", 255, 140, 0);

	if ( can_show_packetloss )
	{
		// PS2HLU
		// height fix
		gHUD.DrawHudString( PL_RANGE_MAX + xpos_rel - 35, ypos - 5, ScreenWidth, "pkt loss", 255, 140, 0 );
	}
	...
	while ( 1 )
	{
		...
		// PS2HLU
		//xpos = KILLS_RANGE_MAX + xpos_rel + 50;
		...
		// PS2HLU
		// draw damage
		//gHUD.DrawHudNumberString(xpos_rel + 275, ypos, xpos_rel + 250, 0, r, g, b);
...
int CHudScoreboard :: DrawPlayers( int xpos_rel, float list_slot, int nameoffset, char *team )
{
	...
	while ( 1 )
	{
		...
		// PS2HLU
		// This is disabled in decay, when only 1 player
		// is playing (the other is a bot)
		...
		else if ( pl_info->thisplayer ) // if it is their name, draw it a different color
		{
			...
			// PS2HLU
			// Corrected blue line height
			FillRGBA( NAME_RANGE_MIN + xpos_rel - 5, ypos + 4, FAR_RIGHT, ROW_GAP, 0, 0, 255, 70 );
		}
		...
		// draw kills (right to left)
		xpos = KILLS_RANGE_MAX + xpos_rel;
		gHUD.DrawHudNumberString( xpos - 50, ypos, KILLS_RANGE_MIN + xpos_rel - 50, g_PlayerExtraInfo[best_player].frags, r, g, b );

		// draw divider
		xpos = DIVIDER_POS + xpos_rel;
		gHUD.DrawHudString( xpos - 55, ypos, xpos + 20 - 55, "/", r, g, b );

		// draw deaths
		xpos = DEATHS_RANGE_MAX + xpos_rel;
		gHUD.DrawHudNumberString( xpos - 55, ypos, DEATHS_RANGE_MIN + xpos_rel -55, g_PlayerExtraInfo[best_player].deaths, r, g, b );

		// PS2HLU
		// draw damage
		gHUD.DrawHudNumberString(xpos_rel + 235, ypos, xpos_rel + 175, g_PlayerExtraInfo[best_player].damageTaken, r, g, b);

		// draw accuracy
		xpos = ((PING_RANGE_MAX - PING_RANGE_MIN) / 2) + PING_RANGE_MIN + xpos_rel + 25;
		static char buf[64];
		sprintf(buf, "%d", g_PlayerExtraInfo[best_player].accuracy);
		gHUD.DrawHudStringReverse(xpos, ypos, xpos - 50, buf, r, g, b);
		gHUD.DrawHudString(xpos, ypos, xpos + 25, "%", r, g, b);
